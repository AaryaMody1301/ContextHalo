...
- Local AI keeps streaming and verified Vulkan/CPU fallback. New/default installs use Qwen 3.5 2B Q4 instead of the older 4B default; the old implicit default migrates once. 0.8B remains the fastest manual choice. Small presets use a 4096-token native context while 4B+ models retain 8192. Phase 4 moves both CPU and Vulkan llama runners to the same official b10964 source revision, so both use the audited `--cache-reuse 256` CLI contract.
