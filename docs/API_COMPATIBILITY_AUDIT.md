...
Current runtime corrections: [Final runtime repair, September 23](FINAL_RUNTIME_REPAIR_2026-09-23.md). Model/dependency versions below are intentionally unchanged.
...
| Gemini errors | SDK retries are disabled (`attempts: 1`); ContextHalo owns the initial HTTP request plus up to four retries inside one 70-second provider budget, with bounded backoff/provider delay. Distinguish 409 ABORTED from ALREADY_EXISTS; cancellation/deadline actively abort work, even a noncooperative SDK | https://ai.google.dev/gemini-api/docs/api-errors |
...
| Electron | 44.3.0; one natively non-resizable transparent window, main-owned DIP resize/fit/restore, sandbox/context isolation, trusted-frame display-media handler, Windows loopback; asynchronous clipboard results supported | https://www.electronjs.org/docs/latest/api/session and https://www.electronjs.org/docs/latest/tutorial/security |
| Browser capture | AudioWorklet with bounded credit/ack messages; 40 ms PCM for Gemini at 16 kHz; 100 ms for Groq/Local; stale/ended capture and processor errors recover visibly; fresh frame counters rather than assuming delay proves freshness | https://developer.mozilla.org/en-US/docs/Web/API/HTMLVideoElement/requestVideoFrameCallback and https://developer.mozilla.org/en-US/docs/Web/API/AudioWorkletProcessor/process |
...
The HTTP retry helper owns a 70-second abortable provider deadline. SDK retries remain disabled, while ContextHalo can make the initial request plus up to four bounded HTTP retries within that one budget; no retry occurs after partial text has already rendered. Typed requests do not carry an independent 27-second per-attempt cap. The screen hierarchy remains 70s provider, 75s session, 77s Windows wrapper, 80s renderer watchdog.
...
`window.js` owns native configuration and all window IPC. `windowModeController.js` owns resize/fit/restore and persisted geometry. The obsolete maximize handler in `runtimeHardeningMain.js` is removed. `WindowResizeHandles.js` reports gestures only. The exact packaged 100% Windows smoke now includes cropped desktop compositor evidence; other scales retain existing layout checks. Test implementation is not a passed hardware/account acceptance result.
