...
| Native inference | Preferred Vulkan llama.cpp is the exact upstream `b10964` release and its CLI/HTTP fields are checked against that tag. Legacy CPU llama/Whisper fallback assets are checksum pinned but their exact upstream build commits are not recorded by the historical release; do not claim exact source ancestry until Phase 4 replaces/rebuilds them | https://github.com/ggml-org/llama.cpp/blob/b10964/tools/server/README.md and https://github.com/ggml-org/whisper.cpp/tree/master/examples/server |
...
Real Gemini/Groq account success, a physical microphone/loopback device, native downloaded-model inference, protected sharing applications, and wall-clock one-hour stability require [device/account acceptance](INTERVIEW_RELIABILITY_ACCEPTANCE.md) and [the cross-provider matrix](RELIABILITY_AUDIT.md). The exact upstream source commits used to build the legacy CPU llama/Whisper fallback executables also remain unproven and are a Phase 4 supply-chain requirement. Do not mark these passed without evidence.
