...
Local verification: the baseline complete suite passed 373/373 with zero skips. New regressions cover terminal Search comparisons with the installed SDK, native backdrop ownership, foreground fading, and compositor/debugger order. Final counts and Windows CI results are recorded in the PR follow-up evidence. The local Electron probe could not load its GPU/renderer child processes (`0xc0000135`), so it provides no native pixel proof.

The compositor result for the new head must be read from the existing Windows workflow. Windows Server CI cannot substitute for representative Windows 10/11 hardware, physical DPI, live provider accounts or long sessions. `audit:readiness` still reports 14 physical scenarios not run and the administrative release gates open.
