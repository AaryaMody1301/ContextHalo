...
Validation of the capture-protection correction against the exact workflow-411 source snapshot:

- `npm run check`: **140 JavaScript files passed**.
- Focused appearance, window geometry, Gemini Search recovery, UI recovery, IPC/security and compositor suites: **86 passed, zero failed, zero skipped**.
- The compositor suite verifies successful black/white control samples, app alpha samples, exact display selection, single-display fallback, control placement outside the app, fixture mismatch, opacity mismatch, empty capture, ambiguous display source, z-order fallback, capture failure, and production/DevTools/native-resize/**content-protection** guards.
- A behavioral appearance test verifies production controllers apply only `true` protection calls while the smoke controller applies none; the final runtime source test locks the smoke-only option wiring in `window.js`.
