...
Only a successful control commits the Live fallback. It establishes that a no-Search connection is available, not whether the underlying cause was tool quota, entitlement or a temporary provider condition. Session details explicitly retain that uncertainty. A failed control is terminal for this attempt; it cannot trigger another control, a core-configuration retry, or false successful Search attribution. Existing bounded core-configuration compatibility recovery remains available when Search is already off.
...
Model defaults and direct dependency pins are unchanged; vulnerable transitive build dependencies were refreshed within their declared version ranges during the PR follow-up. Existing Gemini AUDIO/transcription, thinking omission for Live, compression, cancellation, grounding attribution/retention and thought-signature behavior are preserved. Groq and Local AI routing, credentials, native download provenance, saved History and GPL provenance are not changed. Older dated audits retain their original evidence but are explicitly superseded where their transparency/Search or completion claims conflict with this repair.
...
## PR #71 follow-up audit

The review started from `bae97a724678a894074e62559749b42d6ad33879`, the actual PR head, rather than the older commit/branch in the handoff. Workflow 422 (`35843729837`) passed all 373 tests, source checks, real Electron smoke and packaging, but failed the packaged compositor at alpha 0: the verified black backdrop control was correct and the app crop was white (`255,255,255`). The earlier root-color-scheme change therefore did not establish native transparency.

Corrections in this follow-up:

- Removed `setBackgroundMaterial('none')` from creation and mode transitions. Electron 44.3.0 implements this call on Windows 11 by resetting DWM margins and native translucency; it is not a harmless reset for a transparent window. ContextHalo never enables a system material. The README no longer advertises Mica/Acrylic.
- Moved debugger-based accessibility/emulated-media checks after compositor acceptance. Chromium inspector detachment clears its canvas-background override; a prior debugger attachment is an invalid baseline for this test. The pixel gate retains the same tolerances and black/white controls, and adds the real HUD foreground control. `--ci-smoke-test --compositor-acceptance` also makes this gate available for source runs.
- Every failed Search-off comparison is now terminal for that attempt, including generic WebSocket 1011 and configuration failures. It cannot make a third, different-core connection or repeat the comparison through the outer retry loop. Search-off-only core recovery still has coverage.
- Refreshed vulnerable transitive build dependencies (XML/YAML parsing, Lodash, minimatch, brace-expansion and temporary-file helpers) without changing Electron, the Gemini SDK, marked or model defaults. The full npm audit now reports zero vulnerabilities; the production-only audit was already clean.
- Removed literal backslash-n text between renderer script tags.
- Removed the HTTP request cooldown from the Live reconnect UI guard. An HTTP Search limit could leave the separate Retry connection button enabled but silently prevent it from connecting. The regression verifies Live recovery succeeds while the HTTP error is retained, and that the Live cooldown still blocks reconnect.

Local verification: the baseline complete suite passed 373/373 with zero skips; the first repair passed 379/379 with zero skips and validated 140 JavaScript files. New regressions cover terminal Search comparisons with the installed SDK, native backdrop ownership, foreground fading, and compositor/debugger order. The additional HTTP/Live cooldown regression failed before its one-line correction and passed afterward; the final full suite passed 380/380 with zero failures or skips, and all 140 JavaScript files passed validation. The local Electron probe could not load its GPU/renderer child processes (`0xc0000135`), so it provides no native pixel proof.

[Workflow 423 / run 35850197727](https://github.com/AaryaMody1301/ContextHalo/actions/runs/35850197727) passed for repair head `2420f9625bdb9fa3f37dcfb812c4a22773444d5e` (GitHub PR merge checkout `bcd354fbd32d8bf619a780983ec98ebc72b99d8d`). All 379 regression tests, 140-file source validation, real Electron smoke, portable packaging, provenance checks and packaged launches at 100/125/150/200% Chromium scale passed. The downloaded behavior artifact confirms all ten native compositor captures at scale 1 passed: alpha 0 follows black/white backdrops exactly, alpha 1 is RGB 16 on both, intermediate values blend as expected, and the real HUD End button remains RGB 38 at every alpha/backdrop. The EXE's Authenticode state was `NotSigned`; release publishing and attestation jobs were skipped for this PR. These are CI artifacts, not a published release.

The final cooldown correction requires its own normal Windows workflow result; the PR records that result after completion. Windows Server CI cannot substitute for representative Windows 10/11 hardware, physical DPI, live provider accounts or long sessions. `audit:readiness` still reports 14 physical scenarios not run and the administrative release gates open.

Additional implementation references: [Electron 44.3 native backdrop implementation](https://github.com/electron/electron/blob/v44.3.0/shell/browser/native_window_views.cc), [Chromium inspector cleanup](https://chromium.googlesource.com/chromium/src/+/main/third_party/blink/renderer/core/inspector/inspector_emulation_agent.cc), [brace-expansion advisory](https://github.com/advisories/GHSA-rgw5-rvv9-x895), [js-yaml advisory](https://github.com/advisories/GHSA-2883-xcg3-v3hh).
