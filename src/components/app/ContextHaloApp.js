    ...
    async _prepareSession(epoch, options) {
        ...
        try {
            ...
            if (this.providerMode !== 'local') {
                ...
                const configured = this.providerMode === 'groq' ? credentials?.groq : credentials?.gemini;
                if (!configured) throw new Error(`No ${this.providerMode === 'groq' ? 'Groq' : 'Gemini'} API key configured. Open provider settings.`);
            }
