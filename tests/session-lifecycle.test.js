...
test('failed credential writes block Start until a newer value is securely saved',async()=>{
    ...
    let starts=0; const view=Object.assign(Object.create(Target.prototype),{_mode:'byok',_geminiKey:'',_geminiKeyPresent:false,_geminiLiveModel:'live',_geminiHttpModel:'http',_catalogEpochs:{gemini:0},_catalogTimers:{},_keySavePromise:Promise.resolve(),_configurationWrites:Promise.resolve(),downloadProgress:{active:false},requestUpdate(){},onStart(){starts++;}});
    await view._saveGeminiKey('fixture'); await view._handleStart(); assert.equal(starts,0); assert.equal(view._geminiKey,'fixture');
    fail=false; await view._saveGeminiKey('retry'); assert.equal(view._geminiKey,''); assert.equal(view._geminiKeyPresent,true); await view._handleStart(); assert.equal(starts,1); assert.deepEqual(writes,['fixture','retry']);
});
