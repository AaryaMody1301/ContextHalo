    ...
    const window = {
        webContents: {
            ...
            async executeJavaScript(code) {
                ...
                if (code.includes("const shell=root.querySelector('.app-shell')")) {
                    return { capture: { x: 100, y: 100, width: 88, height: 24 },
                        surface: { x: 124, y: 112 }, foreground: { x: 176, y: 112 } };
                }
    ...
    const api = loadMain('scripts/windows-compositor-acceptance.js', {
        ...
        electron: {
            ...
            nativeImage: { createFromPath: () => ({
                getSize: () => options.emptyImage ? { width: 0, height: 0 } : { width: 132, height: 36 },
                toBitmap() {
                    const bitmap = Buffer.alloc(132 * 36 * 4);
                    const surface = options.opaqueSurface ? 16 : Math.round(16 * alpha + background * (1 - alpha));
                    for (const [x, value] of [[36, surface], [113, options.fadedForeground ? 112 : 224]]) {
                        const offset = (18 * 132 + x) * 4;
...
test('compositor capture uses native BitBlt with layered-window flags and balanced HDC ownership', async () => {
    ...
    assert.equal(execOptions.env.CONTEXTHALO_TEST_RECT, '-1200,210,132,36');
    const setup = f.evaluations.find(code => code.includes("const shell=root.querySelector('.app-shell')"));
    assert.match(setup, /shell\.append\(marker\)/);
    assert.match(setup, /background:rgb\(224,224,224\)/);
    assert.match(setup, /getBoundingClientRect\(\)/);
    assert.match(setup, /:not\(#compositor-test-marker\)/);
...
for (const [name, options, message] of [
    ...
    ['faded foreground', { fadedForeground: true }, /Opaque foreground mismatch/],
