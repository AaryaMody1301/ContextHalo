...
function componentClass(file, name, overrides = {}) {
    ...
    const context = {
        ...
        window: Object.assign(events, { process: { platform: 'win32', arch: 'x64' }, electronAPI: { invoke: async () => ({ success: true }), send() {}, on() {}, removeListener() {}, removeAllListeners() {} } }),
...
function appFixture(options = {}) {
    ...
    loaded.context.window.electronAPI.send = channel => calls.push(channel);
