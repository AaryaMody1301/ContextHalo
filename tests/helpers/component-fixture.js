...
function appFixture(options = {}) {
    ...
    const api = {
        ...
        storage: { getConfig: async () => ({onboarded:true}), getPreferences: async () => ({ providerMode: options.mode || 'byok' }), getKeybinds: async () => ({}), getCredentialStatus: async () => ({ gemini: true, groq: true }) },
